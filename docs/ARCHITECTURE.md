...
**Conversation memory** — `/assistant` accepts a `conversation_id` (a new conversation is created and streamed back otherwise). The model sees the summary plus the most recent messages *on the visible path*; older messages are folded into the summary incrementally after each exchange, and the summary is written onto the message it covers so switching to another variant never inherits the wrong one. Follow-ups need no query rewriting: if the seeded search misses, the model issues its own better-phrased searches with the history in view.

**User memory** — Conversation memory is scoped to one conversation; user memory outlives it. After every run the user's message goes to the fast model with `MEMORY_PROMPT`, a twelve-example prompt that returns one durable fact per line ("The user is a backend developer") or `none` for questions, greetings, and statements about the world. Two deterministic guards bracket the model rather than trusting it: a message whose sentences are all questions, none of them opening as a first-person statement, never reaches the LLM at all — a question is not an assertion, and one stored as a fact is indistinguishable from a real one afterwards — and any extracted fact carrying a placeholder word (something, somewhere, things, stuff) is dropped before it can be stored. Each fact is checked against what is already stored — a similarity search against itself, deduplicated at cosine ≥ 0.95 — and the survivors are written through the `MemoryStore` abstraction, which is mem0 configured with Ollama embeddings, a third Qdrant collection (`memories`), and a SQLite history file in `backend/user_memory/`. Facts are written with `infer=False`: mem0 stores the text verbatim and Cortex owns the extraction, because mem0's own extraction prompt is built for frontier models and a 4B one returns nothing at all against it (see DECISIONS.md). Before every run — including regenerate and edit — the top `user_memory_top_k` facts for the question are recalled and appended to the system message as known facts, with the instruction never to cite them or mention that anything was remembered. Recall is deliberately ungated: query-to-fact scores from an asymmetric embedding model used without its prefixes are not separable, so the model, not a threshold, decides what is relevant. Both directions are wrapped so that a failure in memory can never break a chat. `GET /memories` lists what Cortex believes about the user and `DELETE /memories/{id}` removes one, surfaced as a Memories view in the frontend — necessary rather than decorative, since mem0 2.x is additive only and a superseded fact ("I live in Berlin now") is stored beside the old one rather than replacing it, so correcting the record is a manual act the interface has to allow. `USER_MEMORY_ENABLED=false` substitutes a null store, so the switch costs no branching at the call sites.

**Assistant** — `/assistant` is the single answering endpoint: a LangGraph graph (`route → retrieve → model ⇄ tools`). A `route` node asks the fast model whether the question needs the user's documents, judging follow-ups with the previous user question attached (measured at 100% on a 46-question routing golden set, `evals/routing.py`); document questions run the retrieval funnel with the raw question first — guaranteed grounding, streamed as a normal search step — while live-data, web, arithmetic, small-talk, and Cortex-stats questions jump straight to the model. From there the model orchestrates itself with tool definitions (`search_documents`, `web_search`, `web_image_search`, `web_video_search`, `calculator`, `world_clock`, `get_weather`, `crypto_price`, `kb_stats`, `usage_stats`, `generate_image`): answering directly when the seeded search suffices, decomposing into focused searches when it doesn't, and reaching for the web only when the relevance-gated document searches come back empty. Every step streams over SSE as typed events (`tool_call`, `tool_result`, `sources`, `widget`, `token`, `title`, `usage`, `saved`, `done`), and executed steps persist as JSONB on the assistant message so old chats replay their trace; the `saved` event carries the persisted assistant message id, written before `done` so the client can address the message immediately (conversation summarization runs after `done`). Tools with a visual payload — the world clock, weather, the 24-hour crypto chart, knowledge-base and usage stats, generated images, web image results, the related-image galleries that document searches attach, and web videos (YouTube results as a click-to-load embedded player — metadata only, nothing downloaded) — return a widget alongside their text: streamed as a `widget` event, rendered as a live card under the answer, and persisted on the message like the steps, so reopened chats keep their cards; the model is instructed to answer in one short sentence beside a card rather than repeat its numbers. Every assistant answer takes a like/dislike via `PUT /messages/{id}/feedback` — an idempotent set that also switches or clears the vote (`null`), last state wins — persisted on the message, replayed with the chat, and counted on the dashboard. Any answer can also fork the conversation: `POST /messages/{id}/branch` deep-copies the prefix up to that answer (messages with their traces, sources, widgets, and usage; the summary too, unless the branch point falls inside the summarized region) into a new conversation that records its origin (`branched_from_id`, `branched_count`); the frontend renders a divider at the copy point linking back to the source, and deleting the source sets the link NULL without touching the branch. With conversations now trees, a branch copies the active path and leaves unchosen variants behind. Retrieved passages are numbered globally per run (`[1]`, `[2]`, …) in a sources registry kept in graph state; the model cites passage numbers after claims, and the frontend renders them as chips that expand the exact chunk. Answers from runs that end with no sources are scrubbed of stray citation markers before persisting; the frontend additionally refuses to link any id that matches no source. LangGraph handles only control flow; nodes call the project's own provider abstractions. State checkpoints to Postgres after every node under a per-request thread id, which enables two things. Human-in-the-loop approval: before a `web_search`, `web_image_search`, or `web_video_search` executes, the graph pauses via `interrupt()`, the stream emits an `approval` event and ends, and `POST /assistant/resume` continues the run from the checkpoint with the user's decision. Crash recovery: the conversation row tracks its active thread, and reopening a chat whose stream died calls `POST /assistant/continue`, which replays a `snapshot` of the work so far and re-runs the graph from the last completed node.

**Vision** — Images (`.png`, `.jpg`) are parsed by a local vision model that transcribes text and describes figures; the transcription then flows through the normal chunk → embed → index pipeline. PDFs without a text layer (scans) are rendered page by page and OCR'd the same way, capped at `ocr_max_pages`. `/ask-image` answers a question about an uploaded image directly, without indexing it — the upload is validated and written through a third `FileStore` (`backend/chat_images/`, served at `/chat-images/{filename}`) and recorded in the message's `attachments` column, so a reopened chat shows the picture instead of its filename; the message stores only the filename, as widgets do, and an image that fails validation still gets an answer and degrades to a named file chip; its prompt states that the model can only look, never produce or edit an image, so a request to change a picture is declined rather than role-played, and gemma's image control tokens are stripped in the provider before any answer or caption is used.
...
**Voice** — Both directions run in-process inside the backend, so audio never leaves the machine. `/transcribe` accepts a WAV upload — the chat's mic button captures raw PCM off the microphone with an AudioWorklet and encodes it as WAV client-side, so no compressed codec exists on either side — and hands it to Whisper behind the `SpeechToText` abstraction, which resamples to 16 kHz; the transcript auto-sends through the normal `/assistant` flow, changing nothing downstream. `/speak` turns answer text into WAV through the `TextToSpeech` abstraction (Kokoro), stripping code blocks, markdown symbols, and citation markers first; the frontend plays it behind a read-aloud button on every saved assistant answer, one playback at a time.
...
**Conversation tree** — A conversation is a tree of messages, not a list. `messages.parent_id` makes an answer's siblings its alternatives, so regenerating adds an assistant sibling and editing a question adds a user sibling with its own answer; `n/m` is computed from sibling order at read time. The visible conversation is the path found by following `conversations.active_root_id` and then each message's `active_child_id`, so every fork remembers its own selection and each variant keeps its own continuation, trace, sources, widgets, and vote. `PUT /messages/{id}/select` moves one pointer and returns the new path; `POST /messages/{id}/regenerate` and `POST /messages/{id}/edit` stream new variants through the same graph, SSE events, approval interrupt, and crash recovery as `/assistant` — a run records its parent in `conversations.active_parent_id` so a resumed run attaches where it started. The parent's role alone decides what a finished run saves: under a user message an answer, otherwise a question and an answer, which makes appending and editing the same operation. Conversation summaries live on the messages (`summary`, `summarized_depth`) rather than the conversation row, because a summary is only correct for the path that produced it; history building walks up the path to the nearest ancestor carrying one.

**Storage** — PostgreSQL is the source of truth: documents, chunks, messages (with their attachments), images, logs, and later collections and users. Schema is managed with Alembic. Qdrant stores one vector per chunk in the `chunks` collection, one caption vector per image in the `images` collection, and one vector per remembered fact in the `memories` collection — chunks and images sit in separate collections because their point ids are the Postgres row ids and would collide, while memories are keyed by mem0's own UUIDs and are the one thing Cortex stores that Postgres does not own: the fact text lives in the Qdrant payload and its edit history in `backend/user_memory/`, a deliberate exception recorded in DECISIONS.md. Keeping the stores in sync is the ingestion service's responsibility. Binary files live on disk behind the `FileStore` abstraction; MinIO stays out until a deployment needs it.
...
| `VideoSearchProvider` | ddgs (DuckDuckGo)       | YouTube Data API, Brave      |
...
| `MemoryStore`       | mem0 (Ollama + Qdrant)    | Zep, Letta, a native Qdrant + Postgres store |
...
| gemma3:4b                            | Ollama                            | Routing (`route` node), conversation titles, user-memory fact extraction, and vision: OCR, image captions, `/ask-image` |
| nomic-embed-text                     | Ollama                            | Embeddings for chunks, image captions, remembered facts, and queries                           |
...
.venv/bin/python -m evals.routing              # route node accuracy
.venv/bin/python -m evals.memory               # user-memory fact extraction
```

`evals/memory.json` holds messages paired with the words the extracted facts must contain — an empty list means nothing should be remembered. Run it after any change to `MEMORY_PROMPT` or the extraction model.
