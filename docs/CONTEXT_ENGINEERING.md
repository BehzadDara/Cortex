...
In Cortex: retrieved passages get bracketed numeric IDs, numbered globally per run across every tool call (`offset = len(state["sources"])`, `app/assistant_graph.py:312, 376, 384`), joined with `"\n\n---\n\n"` (`app/assistant_graph.py:169-179`). The frontend renders `[1]`/`[2]` as clickable chips. There's no XML/JSON-level structure beyond that — no `<document>` tags, just numbers plus separators inside a plain-text tool-role message.

### Instruction/Data Separation — Gap
...
### Context Conflict Resolution — Gap
...
In Cortex: mem0 2.x is additive-only — a superseded fact ("I live in Berlin now") is stored *beside* the old one, not replacing it. Cortex's answer is a manual UI escape hatch (`GET /memories` / `DELETE /memories/{id}`, a Memories view), not automatic contradiction detection. A lightweight middle ground: before writing a new fact, check it against existing ones for the same predicate (same subject + attribute, different value) and either replace or flag rather than always appending. Full automatic resolution is probably overkill for a single-user tool; a "this looks like it supersedes an existing fact" prompt to the user is proportionate.
...
In Cortex: `AssistantState` (`app/assistant_graph.py:100-108`) is one flat `TypedDict` — `messages`, `sources`, and `widgets` all use `operator.add` reducers, so every node (`route`, `retrieve`, `model`, `tools`) reads and appends to the same shared lists. There's no per-node scratch state or subgraph boundary in the current graph. This is a reasonable tradeoff, not an oversight — a single shared state is exactly what let the earlier planner/fan-out graph get simplified away. Only worth revisiting if a future node needs to do heavy intermediate work it shouldn't leak into the model's final context.
