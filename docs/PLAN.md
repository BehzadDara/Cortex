...
- [ ] **23. Voice input** — A `SpeechToText` abstraction (Whisper large-v3-turbo via mlx-whisper, loaded in-process like the reranker), a `/transcribe` endpoint, and a mic button in the chat that records audio and drops the transcript into the normal `/assistant` flow. Audio never leaves the machine.
- [ ] **24. Voice output** — A `TextToSpeech` abstraction (Kokoro via kokoro-onnx, weights in `backend/voice_models/`), an endpoint that turns an answer into audio, and a read-aloud button on assistant messages.
