...
- [x] **20. Image generation** — A `generate_image` tool behind an `ImageGenerator` abstraction; the image lands on disk served at `/images`, and the answer shows it as an image card widget, persisted and replayed like the others. Built first on Ollama's experimental image API (`x/z-image-turbo` locally), which proved too heavy beside the chat models on 24 GB — the default became the free keyless Pollinations API (~2 s per image), with the local Ollama implementation one `IMAGE_PROVIDER` switch away. Draw-requests route straight to the model; the routing set grew to 42 questions — 100%.
